...
const Coin = (props) => {
  ...
  /*let profit2 = coin.market_data.price_change_percentage_24h_in_currency.usd >= 0; 
  let profit3 = coin.market_data.price_change_percentage_7d_in_currency.usd >= 0; 
  let profit4 = coin.market_data.price_change_percentage_14d_in_currency.usd >= 0; 
  let profit5 = coin.market_data.price_change_percentage_30d_in_currency.usd >= 0; 
  let profit6 = coin.market_data.price_change_percentage_1y_in_currency.usd >= 0; */
        ...
        <div className='content'>
            ...
            <div className='info'>
                ...
                <div className='coin-price'>
                    {coin.market_data?.current_price ? <h1>${millify(coin.market_data.current_price.usd)}</h1> : null}
                </div>
            </div>
        </div>
        ...
        <div className='content'>
            <div className='stats'>
                <div className='left'>
                    <div className='row'>
                        <h4>24 Hour Low</h4>
                        {coin.market_data?.low_24h ? <p>${millify(coin.market_data.low_24h.usd)}</p> : null}
                    </div>
                    <div className='row'>
                        <h4>24 Hour High</h4>
                        {coin.market_data?.high_24h ? <p>${millify(coin.market_data.high_24h.usd)}</p> : null}                            </div>

                </div>
                <div className='right'>
                    <div className='row'>
                        <h4>Market Cap</h4>
                        {coin.market_data?.market_cap ? <p>${millify(coin.market_data.market_cap.usd)}</p> : null}
                    </div>
                    <div className='row'>
                        <h4>Circulating Supply</h4>
                        {coin.market_data ? <p>{millify(coin.market_data.circulating_supply)}</p> : null}
                    </div>
